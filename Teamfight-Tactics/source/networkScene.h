...
public:
    ...
    void detect_attack_in(float dt) {
        ...
        if (tag1 && tag2 && tag4) {  //成功构成攻击羁绊
            this->unschedule(CC_SCHEDULE_SELECTOR(networkScene::detect_attack_in));
            //显示图标、响起音效
            attacktram->setVisible(true);
            int audioId = AudioEngine::play2d("trammel.mp3");
            AudioEngine::setVolume(audioId, 1.0f);
            this->schedule(CC_SCHEDULE_SELECTOR(networkScene::detect_attack_out), 0.01f);
        }
    }
    void detect_attack_out(float dt) {
        ...
        if ((!tag1) || (!tag2) || (!tag4)) {  //成功退出攻击羁绊
            this->unschedule(CC_SCHEDULE_SELECTOR(networkScene::detect_attack_out));
            //撤去图标
    ...
    void detect_heal_in(float dt) {
        ...
        if (tag3 && tag5 && tag6) {  //成功构成恢复羁绊
            this->unschedule(CC_SCHEDULE_SELECTOR(networkScene::detect_heal_in));
            //显示图标、响起音效
            healtram->setVisible(true);
            int audioId = AudioEngine::play2d("trammel.mp3");
            AudioEngine::setVolume(audioId, 1.0f);
            this->schedule(CC_SCHEDULE_SELECTOR(networkScene::detect_heal_out), 0.01f);
        }
    }
    void detect_heal_out(float dt) {
        ...
        if ((!tag3) || (!tag5) || (!tag6)) {  //成功退出恢复羁绊
            this->unschedule(CC_SCHEDULE_SELECTOR(networkScene::detect_heal_out));
            //撤去图标
